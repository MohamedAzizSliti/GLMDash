...
export class TableComponent {
  ...
  onRowClicked(rowData: any): void {
    if(this.hasPermission(['edit', 'view']) ||  this.hasPermission(['view-exams'] )) {
  ...
  getNestedValue(data: any, field: string): any {
    // Split the string by '.' to handle nested properties
    return field.split('.').reduce((acc, part) => acc && acc[part], data);
  }
  isNested(field: string): boolean {
