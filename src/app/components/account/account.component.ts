...
export class AccountComponent {
  ...
  constructor(private store: Store, private formBuilder: FormBuilder) {
    this.user$.subscribe(user => {
      ...
      if(user?.company) {
        this.form = this.formBuilder.group({
          store_name: new FormControl(user.company.school_name, [Validators.required]),
          description: new FormControl(user.company.description, [Validators.required]),
          country_id: new FormControl(user.company?.country_id, [Validators.required]),
          state_id: new FormControl(user?.company?.state_id, [Validators.required]),
          city: new FormControl(user?.company?.city, [Validators.required]),
          address: new FormControl(user?.company?.address, [Validators.required]),
          pincode: new FormControl(user?.company?.pincode, [Validators.required]),
          store_logo_id: new FormControl(user?.company?.company_logo_id),
          hide_vendor_email: new FormControl(user?.company?.hide_client_email),
          hide_vendor_phone: new FormControl(user?.company?.hide_client_phone),
          facebook: new FormControl(user?.company?.facebook),
          instagram: new FormControl(user?.company?.instagram),
          pinterest: new FormControl(user?.company?.pinterest),
          youtube: new FormControl(user?.company?.youtube),
          twitter: new FormControl(user?.company?.twitter),
        })
      }
